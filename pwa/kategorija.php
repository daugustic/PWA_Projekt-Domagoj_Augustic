...
<body>
    ...
    <section class="Laptopi">
        ...
        <?php
        $query_laptopi = "SELECT * FROM proizvod WHERE kategorija='Laptop' ";
        ...
        while ($row = mysqli_fetch_array($result_laptopi)) {
            ...
            echo '<h4 class="title">';
            echo '<a href="proizvod.php?id=' . $row['id'] . '">' . $row['naziv'] . '</a>';
            echo '</h4>';
            echo '</div></div>';
            echo '</article>';
        }
    ...
    <h2>Mobiteli</h2><br><br>
    <section class="Mobiteli">
        
        <?php
        $query_mobiteli = "SELECT * FROM proizvod WHERE kategorija='Mobitel'  ";
